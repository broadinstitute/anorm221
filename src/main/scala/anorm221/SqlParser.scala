...
object SqlParser {
  ...
  def flatten[T1, T2, R](implicit f: anorm221.TupleFlattener[(T1 ~ T2) => R]): ((T1 ~ T2) => R) = f.f

  def str(columnName: String): RowParser[String] = get[String](columnName)(implicitly[anorm221.Column[String]])
  ...
  def getAliased[T](aliasName: String)(implicit extractor: anorm221.Column[T]): RowParser[T] = RowParser { row =>
    import MayErr._
  ...
  def get[T](columnName: String)(implicit extractor: anorm221.Column[T]): RowParser[T] = RowParser { row =>
    import MayErr._
